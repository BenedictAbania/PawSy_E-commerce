...
const Products = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const mockData = [
        ...
        { id: 102, name: 'Cat Tree Tower', category: 'Furniture', price: 120.00, stock: 5, image: imgCatTower },
        { id: 103, name: 'Squeaky Bone', category: 'Toys', price: 12.50, stock: 0, image: imgBoneToy },
      ];
      if (products.length === 0) setProducts(mockData);
  ...
  const handleSave = async () => {
    ...
    const productPayload = { ...currentProduct, image: finalImage };

    try {
        if (isEditing) {
            setProducts(products.map(p => p.id === currentProduct.id ? { ...productPayload, price: parseFloat(currentProduct.price) } : p));
        } else {
            setProducts([...products, { ...productPayload, id: Date.now(), price: parseFloat(currentProduct.price) }]);
        }
        setShowModal(false);
    } catch (error) {
